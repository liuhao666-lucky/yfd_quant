...
| 品种 | 表 | 字段 |
|------|-----|------|
| NQ 期货 | `nq_daily` | date, open, high, low, close |
| NDX 现货 | `ndx_daily` | date, open, high, low, close, volume |
| VIX 期货 | `vix_daily` | date, open, high, low, close |
| CPO 概念 | `cpo_daily` | date, open, high, low, close, volume, amount |
| USDCNH | `fx_daily` | date, close |

推送报告含：市场收盘数据总览 + 模型预测验证 + 近期表现统计。
